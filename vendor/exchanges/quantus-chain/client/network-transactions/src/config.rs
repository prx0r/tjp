...
/// Maximum notifications sent to one peer in a single propagate pass.
///
/// The primary guard against overflowing the peer's synchronous notification channel (2048
/// messages on litep2p, where an overflow force-closes the whole connection) is the capacity
/// reported by `NotificationService::sync_notification_capacity`. This constant bounds a single
/// pass on top of that, and is the fallback budget when the backend cannot report capacity.
pub(crate) const MAX_TRANSACTIONS_PER_PROPAGATION: usize = 1024;
